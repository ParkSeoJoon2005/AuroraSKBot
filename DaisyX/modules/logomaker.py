...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./DaisyX/resources/Chopsic.otf", 950)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("DaisyX/resources/*")
    ...
    if os.path.exists(font_):
        if not font_.startswith("DaisyX/resources/"):
            os.remove(font_)    
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./DaisyX/resources/font.otf", 170)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./DaisyX/resources/spiderbg.png')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 25
    fillcolor = "gold"
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./DaisyX/resources/spiderbg (2).png')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 25
    fillcolor = "gold"
